...
export default function detEmpAct() {
    ...
    const fetchActivities = async () => {
      try {
        ...
        if (storedUserData) {
          ...
          const response = await fetch(`http://192.168.100.11:3000/user-activities/${parsedUserData.user_id}`);
  ...
  const handleStartActivity = async (activityId) => {
    try {
      const response = await fetch(`http://192.168.100.11:3000/start-activity/${activityId}`, {
        method: 'PUT',
        headers: {
  ...
  const handleFinishActivity = async (activityId) => {
    try {
      const response = await fetch(`http://192.168.100.11:3000/end-activity/${activityId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
