    ...
    const fetchUserData = async () => {
      try {
        ...
        if (storedUserData) {
          ...
          const response = await fetch(`http://10.13.0.115:3000/user-data?userId=${parsedUserData.user_id}`);
  ...
  const handleLogout = async () => {
    try {
      ...
      if (userData) {
        ...
        const response = await fetch('http://10.13.0.115:3000/logout', {
          method: 'POST',
          headers: {
  ...
  const handleDeleteAccount = async () => {
    try {
      ...
      if (userData) {
        ...
        const response = await fetch('http://10.13.0.115:3000/user-delete', {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
          },
