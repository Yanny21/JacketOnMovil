...
admin.initializeApp({
  ...
  databaseURL: 'https://alpha-prime-873c5.firebaseio.com' // Replace with your Firebase database URL
});

const uwu = admin.firestore();
...
app.get('/generate-report-chart', async (req, res) => {
  try {
      ...
      options: {
        scales: {
          ...
          y: {
            beginAtZero: true,
            ticks: {
              stepSize: 500, // Set y-axis increments to 500 units
              callback: function(value) {
                return value; // Format labels to show as numbers
              }
            }
          }
        }
    ...
    // Create PDF
    const doc = new pdfkit();
    const filePath = './report-chart.pdf';
    doc.pipe(fs.createWriteStream(filePath));
